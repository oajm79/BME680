...
### 🎉 Major Features - SQLite & Telegram

#### SQLite Database Storage
- **`database.py`** - New SQLite-based data storage:
  - Replaces CSV for better querying and data integrity
  - Automatic table creation with proper indexes
  - Stores temperature, humidity, pressure, gas resistance, air quality, and comfort levels
  - Built-in statistics queries (min/max/avg over time periods)
  - Automatic cleanup of old data (configurable retention)
  - Export to CSV functionality for compatibility
  - Alerts table for tracking sent notifications

#### Telegram Notifications
- **`telegram_notifier.py`** - Complete Telegram bot integration:
  - Air quality alerts (when quality becomes poor)
  - Temperature alerts (high/low thresholds)
  - Humidity alerts (high/low thresholds)
  - Daily summary reports with 24h statistics
...
- `sensor.py` now supports both SQLite and CSV storage modes
- Added alert checking on each sensor reading
- Daily tasks: summary reports and database cleanup
